...
- 举例：
	...
	```mermaid
	graph LR
	    TaskManager["TaskManager（使用者）"] --> FileLogger["FileLogger（具体实现）"]
	```
	- 正确依赖 (ILogger是通用日志组建的接口)：
	```mermaid
	graph LR
	    TaskManager["TaskManager"] --> ILogger["ILogger（定义接口的一方）"]
	    FileLogger["FileLogger"] --> ILogger
	```
...
### std::atomic
- C++11引入的一个模板类，用于执行原子操作；
- 核心特性
	- 原子性：操作不可中断。一个线程在对原子变量进行操作时，其他线程要么看到操作前的状态，要么看到操作后的状态；
	- 线程安全：**线程安全**：不需要使用 `std::mutex`（互斥锁）即可实现简单的计数器或标志位，且性能通常优于锁；
	- 内存顺序控制：除了保证原子性，它还负责处理编译器和 CPU 的指令重排问题（Memory Barrier）。
- <mark style={{background: '#FF5582A6'}}>重要</mark>：虽然原子操作比 `std::mutex` 快，但它只能保护**单个变量**。如果需要保护复杂的数据结构读写一致，还是要用锁；
	- 原因：cpu可以在一个时钟周期内，对一个内存位置完成读-改-写；
	- 原子操作利用上述特性，可以在一个cpu指令中锁定单个变量的地址，无法在同一个原子操作中锁定两个不连续的内存地址；
### std::mutex
- 互斥锁
- 作用：同一时刻，只允许一个线程访问被保护的资源；
- 核心操作：
	- lock(): 尝试获取锁。如果锁已被其他线程占用，当前线程会**阻塞**（挂起），直到锁被释放。
	- unlock():释放锁，让其他等待的线程有机会获取它。